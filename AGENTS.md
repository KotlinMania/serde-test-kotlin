...
- **All current KotlinMania targets build** (§0 condition 3 + §5
  ...
  Mach-O 24-bit scattered relocation limit), the JetBrains-deprecated
  x86_64 simulator targets `tvosX64` / `watchosX64` / `macosX64`
  (§5.5.2), and the 32-bit Android Native targets `androidNativeArm32` /
  `androidNativeX86` (§5.5.3, retired 2026-06-25 — many upstream Rust
  crates only provide 64-bit FFI; `size_t` width mismatch between
  32-bit `UInt` and 64-bit `ULong` breaks `posixMain` shared signatures).
  These are workspace-wide retirements, not per-repo shrinks.
...
the surviving 32-bit native targets were all Android Native. As of
2026-06-25 the 32-bit Android Native targets (`androidNativeArm32`,
`androidNativeX86`) are also retired (§5.5.3) for the same `size_t`
width mismatch reason compounded by 64-bit-only FFI in upstream Rust
crates. The rule remains for any future 32-bit target that may be
added; only the offending target list is now empty.
